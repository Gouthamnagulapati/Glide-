...
export default function GlideEngineDashboard() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#030611] text-zinc-400">
      <motion.div 
        className="fixed inset-0 z-0 bg-[radial-gradient(circle_at_50%_50%,_#1a1f35_0%,_#030611_70%)]"
      ...
      <div className="relative z-10 flex flex-col lg:flex-row min-h-screen w-full">
        ...
        <main className="flex-1 p-6 lg:p-16 overflow-y-auto">
          {view === 'dashboard' ? (
            <div>
              <h1 className="text-white text-3xl lg:text-6xl font-black mb-8 lg:mb-16 uppercase tracking-tighter">SYSTEM COMMAND CENTER</h1>
              <div className="bg-[#030611]/30 border border-white/10 rounded-xl p-6 lg:p-10 backdrop-blur-2xl">
                {apps.map((app) => (
                  <div key={app.id} onClick={() => setSelectedApp(app)} className="grid grid-cols-1 lg:grid-cols-3 py-6 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors items-center">
                    <span className="text-lg text-white font-medium mb-2 lg:mb-0">{app.node}</span>
                    <span className="text-zinc-500 mb-2 lg:mb-0">{app.trans}</span>
